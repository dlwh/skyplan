...
\author{David Hall, Aloni Cohen, David Burkett, \and Dan Klein \\
EECS CS Division\\
...
* Before Claim 3, you could briefly recall the definition of "optimally
  efficient" and what it means in this particular setting.

* Definitions 3 and 4: In both definitions, part (2) is just the
  contraposition of (1) and hence implied and unnecessary.

* First paragraph after proof of Claim 4: "In this case, we would not be able
  to prune based on the number of widgets: its resource would be in R=". Maybe
  I just don't see it, but shouldn't it be in R+?

* Last paragraph before Def. 6: One of the n's should be an m.

* Could condition 2 of Def. 6 be cast in a more formal way?


* Experiments: "The implementations of Skyplan and \Astar used in this domain are
  not using the Hierarchical implementation discussed above, but rather a
  critical path heuristic". Why?
...
\begin{algorithm}[t!]
  \begin{algorithmic}[1]
    ...
    \State $n\rightarrow \textit{Closed}$
      ...
      \EndIf
      \If {$\forall m \in \textit{Open} \cup \textit{Closed}. n \nprec m$}
...
states $m$ and $n$ with $m \ne n$. XXX maybe comment on whether a trivial partial
order need exist
...
\begin{table}
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{Results on sequential domains.}
	\label{tab:seq-opt}
\end{table}
...
\begin{table}
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{Results on temporal satisficing domains.}
	\label{tab:tempo-sat}
\end{table}
...
We empirically evaluated Skyplan in two settings. First, we ran experiments on a collection
of standard tasks from the 2008 International Planning Competition. The
woodworking domain. We chose this domain because it
